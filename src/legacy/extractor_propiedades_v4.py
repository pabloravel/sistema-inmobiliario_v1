...
def procesar_propiedad(id_prop: str, datos: Dict) -> Dict:
    ...
    descripcion = ""
    campos_descripcion = ["description", "description_raw", "descripcion", "descripcion_raw", "desc", "texto"]
...
def procesar_archivo():
    ...
    try:
        ...
        for id_prop, datos in propiedades_dict.items():
            try:
                if id_prop != "None":
                    # Obtener campos asegurándonos de que existan
                    descripcion = str(datos.get("description", "")).strip()
                    titulo = str(datos.get("titulo", "")).strip()
                    precio = str(datos.get("precio", "")).strip()
                    location = str(datos.get("location", "")).strip()
