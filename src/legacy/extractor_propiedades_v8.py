...
def extraer_ubicacion(texto: str, location: str = "", ciudad: str = "") -> Dict[str, str]:
    ...
    # Detectar zona con patrones mejorados
    ubicacion["zona"] = obtener_zona_conocida(texto_completo)
    
    # Diccionario de ciudades de Morelos
    ciudades_morelos = {
    ...
    # Patrones mejorados para detectar ciudad
    patrones_ciudad = [
        # Patrones existentes
        ...
        # Nuevos patrones
        r'(?:cerca|junto|próximo)\s+(?:a|de|al)\s+(?:centro\s+de\s+)?([a-zá-úñ\s]+?)(?:,|\.|$)',
        r'(?:en\s+la\s+zona\s+(?:de|del)\s+)?([a-zá-úñ\s]+?)(?:,|\.|$)',
        r'(?:ubicad[oa]\s+en\s+el\s+)?([a-zá-úñ\s]+?)(?:,|\.|$)',
        r'(?:sobre|frente\s+a)\s+(?:la\s+)?(?:avenida|calle)\s+[^,]+?,\s*([a-zá-úñ\s]+?)(?:,|\.|$)',
        r'(?:a\s+(?:unos|pocos)\s+minutos\s+de)\s+([a-zá-úñ\s]+?)(?:,|\.|$)'
    ]
...
def validar_precio_por_zona(precio: float, zona: str, tipo_operacion: str) -> bool:
    ...
    rangos_precio = {
        "Zona Dorada": {
            "venta": (6_000_000, 60_000_000),
            "renta": (20_000, 200_000)
        },
        "Zona Norte": {
            "venta": (3_000_000, 30_000_000),
            "renta": (12_000, 120_000)
        },
        "Zona Sur": {
...
def obtener_puntos_interes(texto: str) -> List[Dict[str, str]]:
    ...
    # Puntos de interés expandidos y optimizados
    puntos_interes = {
        "comercial": {
            "centros_comerciales": [
                ...
                "plaza diana", "plaza capiri", "plaza degollado",
                "plaza san diego", "superama", "chedraui",
                
                # Nuevas plazas comerciales
                "plaza forum", "forum cuernavaca", "plaza solidaridad",
                "plaza lomas", "plaza vista hermosa", "plaza dorada",
                "plaza las américas", "plaza las americas"
            ],
            "mercados": [
                "mercado adolfo lópez mateos", "mercado alta vista",
                "mercado central", "mercado lagunilla", "mercado carolina",
                "mercado amatitlan", "mercado juarez", "mercado municipal",
                "mercado tejalpa", "mercado progreso"
            ]
        },
        "salud": {
            "hospitales": [
                ...
                "hospital medsur", "medsur", "hospital parres",
                
                # Nuevas unidades médicas
                "clinica del imss", "issste chipitlan", "cruz roja morelos",
                "hospital angeles", "hospital cuernavaca", "sanatorio henri dunant",
                "centro médico", "centro medico", "hospital del niño",
                "hospital de la mujer"
            ],
            "farmacias": [
                "farmacia guadalajara", "farmacias del ahorro",
                "farmacia del ahorro", "similares", "farmacias similares",
        ...
        "educativo": {
            "universidades": [
                ...
                # Nuevos campus
                "universidad morelos", "universidad mexicana",
                "universidad internacional", "universidad cuauhnahuac",
                "universidad stratford", "universidad loyola"
            ],
            "escuelas": [
                "boston", "williams", "williams college", "harkness",
                "colegio williams", "colegio boston", "montessori",
                "colegio montessori", "colegio morelos", "colegio cristobal colon",
                "colegio marista", "colegio fray luis de leon",
                "preparatoria federal", "cetis", "conalep"
            ]
        },
        "transporte": {
            "avenidas": [
                ...
                "domingo diez", "poder legislativo", "atlacomulco",
                "boulevard cuauhnahuac", "avenida revolucion"
            ],
            "terminales": [
                "terminal de autobuses", "central camionera",
                "pullman de morelos", "terminal estrella de oro",
                "terminal sur", "terminal casino de la selva"
            ]
        },
        "recreativo": {
            "parques": [
                ...
                "barranca", "parque melchor ocampo", "parque san miguel",
                "parque tlaltenango", "jardín borda", "jardin borda",
                "parque revolucion"
            ],
            "deportivo": [
                "unidad deportiva", "deportiva", "alberca olimpica",
                "gimnasio", "sports world", "sports plaza",
                "club deportivo", "canchas deportivas", "estadio coruco diaz",
                "centro acuático", "centro acuatico"
            ]
        }
    }
