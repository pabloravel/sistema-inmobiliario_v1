...
def extraer_tipo_propiedad(texto):
    ...
    # Patrones que indican que NO es un terreno puro
    no_es_terreno = [
        r'casa\s+(?:con|en)\s+terreno',
        r'propiedad\s+(?:con|en)\s+terreno',
        r'construccion\s+(?:con|en)\s+terreno',
        r'edificacion\s+(?:con|en)\s+terreno'
    ]
    ...
    # Verificar si hay indicadores de que no es un terreno puro
    tiene_construccion = any(re.search(patron, texto, re.IGNORECASE) for patron in no_es_terreno)

    # Si hay más indicadores de terreno y no hay indicadores de que no es terreno puro
    if count_terreno > 0 and count_terreno > count_casa and not tiene_construccion:
        return "terreno"
