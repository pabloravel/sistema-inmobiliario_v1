...
def extraer_tipo_propiedad(texto):
    ...
    # Primero detectar por características físicas evidentes
    if any(re.search(patron, texto) for patron in [
        ...
        # Si tiene características de vivienda, determinar si es casa o departamento
        if any(re.search(patron, texto) for patron in [
    ...
    # Si no hay características físicas claras, buscar palabras clave en el título
    if any(re.search(patron, texto) for patron in [
        r'terreno(?:\s+en\s+construccion|\s+baldio|\s+rustico)?',
        r'lote(?:\s+de\s+terreno)?',
        r'predio(?:\s+rustico)?',
        r'm2\s+de\s+terreno',
        r'metros?\s+(?:cuadrados?\s+)?de\s+terreno',
        r'hectareas?\s+de\s+terreno'
    ]) and not any(re.search(patron, texto) for patron in [
        r'casa(?:\s+en\s+venta)?',
        r'propiedad(?:\s+en\s+venta)?',
        r'inmueble(?:\s+en\s+venta)?',
        r'departamento(?:\s+en\s+venta)?',
        r'edificio(?:\s+en\s+venta)?'
    ]):
        return 'Terreno'
        
    # Si menciona explícitamente casa o departamento
    if any(re.search(patron, texto) for patron in [
        r'departamento|depto|dpto|apartamento|apto',
        r'condominio\s+vertical'
    ]):
    ...
    if any(re.search(patron, texto) for patron in [
        r'casa(?:\s+(?:sola|nueva|usada|remodelada|en\s+venta))?',
