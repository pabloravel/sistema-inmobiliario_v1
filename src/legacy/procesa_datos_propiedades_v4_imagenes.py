...
def normalizar_texto(texto: str) -> str:
    ...
    # Reemplazar frases específicas para los look-behind
    texto = texto.replace("casa con ", "casa_con_")
    texto = texto.replace("casa en ", "casa_en__")
    texto = texto.replace("propiedad con ", "casa_con_")
    texto = texto.replace("propiedad en ", "casa_en__")
...
def extraer_tipo_propiedad(texto):
    ...
    # Detectar departamentos primero por ser más específicos
    caracteristicas_depto = [
        ...
        r'edificio\s+(?:de|con|nuevo)',
        r'torre\s+(?:de|con|nueva)',
        r'elevador',
        r'penthouse',
        r'pent\s*house',
        r'loft',
    ...
    caracteristicas_condominio = [
        ...
        r'casa\s+club',
        r'amenidades',
        r'desarrollo\s+residencial',
        r'conjunto\s+residencial',
        r'privada\s+residencial',
        r'coto\s+privado',
        r'casa\s+en\s+privada'
    ]
    ...
    # Detectar terrenos - características físicas específicas
    caracteristicas_terreno = [
        ...
        r'uso\s+de\s+suelo',
        r'terreno\s+(?:plano|regular|irregular)',
        r'terreno\s+(?:urbano|habitacional|comercial)',
        r'(?:terreno|lote)\s+bardeado',
        r'(?<!casa_con_)terreno\s+de',
        r'(?<!casa_en__)terreno\s+en',
        r'superficie\s+total\s+del\s+terreno'
    ]

    # Verificar si hay menciones explícitas de construcción o casa
    indicadores_casa = [
        r'casa\s+(?:nueva|recién\s+construida|remodelada)',
        r'construcción\s+(?:nueva|reciente)',
        r'\d+\s+(?:recámaras|habitaciones|baños|niveles)',
        r'sala\s+comedor',
        r'cocina\s+integral',
        r'casa\s+(?:amplia|espaciosa)',
        r'metros\s+de\s+construcción',
        r'm2\s+de\s+construcción',
        r'estancia\s+familiar',
        r'área\s+de\s+lavado',
        r'closets?\s+vestidor',
        r'cochera\s+techada',
        r'cuarto\s+de\s+servicio',
        r'estudio\s+o\s+biblioteca'
    ]

    # Contar menciones de características
    count_terreno = sum(1 for patron in caracteristicas_terreno if re.search(patron, texto, re.IGNORECASE))
    count_casa = sum(1 for patron in indicadores_casa if re.search(patron, texto, re.IGNORECASE))

    # Si hay indicadores fuertes de casa, ignorar los indicadores de terreno
    if count_casa >= 2:  # Si hay al menos dos características de casa
        return "casa_sola"

    # Si solo hay indicadores de terreno y no hay indicadores fuertes de casa
    if count_terreno > 0 and count_casa == 0:
        return "terreno"

    # Por defecto casa sola si menciona casa y no es condominio
    if re.search(r'\bcasa\b', texto, re.IGNORECASE) or count_casa > 0:
        return "casa_sola"
