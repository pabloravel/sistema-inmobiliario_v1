...
def extraer_tipo_propiedad(texto):
    ...
    # Primero detectar materiales de construcción o servicios
    if any(re.search(patron, texto) for patron in [
        r'venta\s+de\s+(?:adoquin|material|block|tabique|cemento)',
        r'materiales?\s+(?:de|para)\s+construccion',
    ...
    menciones_casa = [
        r'(?:hermosa|bonita|amplia|nueva)?\s*casa(?:\s+(?:sola|nueva|usada|remodelada|en\s+venta))?',
        r'remato\s+casa',
        r'casa\s+(?:con|en)',
        r'chalet|villa|residencia',
        r'bungalow'
    ]
    ...
    # Si tiene características de vivienda o menciones explícitas de casa/depto
    if (any(re.search(patron, texto, re.IGNORECASE) for patron in caracteristicas_vivienda) or
        any(re.search(patron, texto, re.IGNORECASE) for patron in menciones_casa + menciones_depto)):
        
        # Determinar si es casa o departamento
        if any(re.search(patron, texto, re.IGNORECASE) for patron in menciones_depto):
            return 'Departamento'
        return 'Casa'
    
    # Si no hay características de vivienda, buscar indicadores de terreno
    if any(re.search(patron, texto, re.IGNORECASE) for patron in [
        r'terreno(?:\s+(?:plano|baldio|rustico))?',
        r'lote(?:\s+de\s+terreno)?',
        r'predio(?:\s+rustico)?',
        r'hectareas?\s+de\s+terreno',
        r'uso\s+de\s+suelo'
    ]) and not any(re.search(patron, texto, re.IGNORECASE) for patron in menciones_casa + menciones_depto):
        return 'Terreno'
