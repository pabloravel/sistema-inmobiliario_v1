...
def extraer_tipo_operacion(texto, precio=None):
    ...
    if precio:
        try:
            ...
            # Rangos de precios más realistas para México
            # Si el precio es mayor a 1 millón, es casi seguro que es venta
            if valor_precio >= 1_000_000:
                return "venta"
            # Si el precio está entre 300,000 y 1 millón, necesitamos más contexto
            elif 300_000 <= valor_precio < 1_000_000:
                # Buscar palabras clave adicionales en el texto
                if any(palabra in texto for palabra in ["mensual", "al mes", "/mes", "por mes", "mensualidad"]):
                    return "renta"
                else:
                    return "venta"  # Por defecto asumimos venta en este rango
            # Si el precio es menor a 300,000, podría ser renta o venta
            else:
                if any(palabra in texto for palabra in ["mensual", "al mes", "/mes", "por mes", "mensualidad"]):
                    return "renta"
                return "desconocido"
    ...
    # Si no hay mención explícita ni precio válido, retornar desconocido
    return "desconocido"
