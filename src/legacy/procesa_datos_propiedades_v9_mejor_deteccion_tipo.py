...
def extraer_tipo_propiedad(texto):
    ...
    # Primero detectar terrenos por ser más específicos
    patrones_terreno = [
        r'(?:vendo|venta\s+de)\s+terreno',
        r'terreno\s+(?:de|en|con|para)',
        r'lotes?\s+(?:residenciales?|en|de|con)',
        r'metros?\s+(?:cuadrados?)?\s+(?:de\s+)?terreno',
        r'm2\s+(?:de\s+)?terreno',
        r'mts2?\s+(?:de\s+)?terreno',
        r'precio\s+por\s+metro',
        r'(?:terreno|lote)\s+(?:\d+\s*)?(?:x|por)\s*\d+',
        r'ejidal(?:es)?',
        r'escrituras?\s+(?:en\s+)?regla',
        r'cesion\s+(?:y|de)\s+derechos'
    ]
    
    if any(re.search(patron, texto) for patron in patrones_terreno):
        return 'Terreno'
    
    # Detectar bodegas y locales comerciales
    patrones_comercial = [
        r'bodega\s+(?:en|con|de)',
        r'local\s+comercial',
        r'uso\s+(?:de\s+)?suelo\s+comercial',
        r'(?:zona|área)\s+comercial'
    ]
    
    if any(re.search(patron, texto) for patron in patrones_comercial):
        return 'Comercial'
    
    # Detectar características físicas de vivienda
    caracteristicas_vivienda = [
        r'(?:casa|propiedad)\s+(?:sola|nueva|en|con)',
        r'casa\s+(?:de|en)\s+(?:dos|tres|2|3)\s+niveles',
        r'residencia(?:l)?',
    ...
    caracteristicas_depto = [
        r'departamento|depto|dpto|apartamento|apto',
        r'edificio|torre',
        r'elevador|ascensor',
        r'penthouse|pent\s*house',
        r'loft',
        r'condominio\s+vertical',
        r'desarrollo\s+vertical',
        r'\d+(?:er|do|ro|to|vo|°)?\s*(?:piso|nivel)',
        r'(?:piso|nivel)\s+\d+'
    ]
    
    es_vivienda = any(re.search(patron, texto) for patron in caracteristicas_vivienda)
    es_depto = any(re.search(patron, texto) for patron in caracteristicas_depto)
    
    if es_vivienda and not es_depto:
        return 'Casa'
    elif es_depto:
        return 'Departamento'
    
    # Si no se detectó ningún patrón específico, intentar inferir del título
    titulo_lower = texto.split('\n')[0].lower() if '\n' in texto else texto.lower()
    
    if 'casa' in titulo_lower and not any(x in titulo_lower for x in ['departamento', 'depto', 'dpto', 'apartamento', 'apto']):
        return 'Casa'
    elif any(x in titulo_lower for x in ['departamento', 'depto', 'dpto', 'apartamento', 'apto']):
        return 'Departamento'
