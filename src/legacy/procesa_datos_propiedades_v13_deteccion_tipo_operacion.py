    ...
    try:
        ...
        for id_propiedad, datos in datos_crudos.items():
            try:
                ...
                # Si no hay tipo de operación o es inválido, extraerlo del texto
                if not tipo_op or tipo_op not in ["venta", "renta"]:
                    texto_completo = (titulo + " " + descripcion).lower()
                    tipo_op = extraer_tipo_operacion(texto_completo)
