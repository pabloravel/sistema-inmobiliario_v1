...
def extraer_tipo_propiedad(texto):
    ...
    # Primero buscar menciones explícitas
    menciones_explicitas = {
        'Casa': [
            r'(?:hermosa|bonita|preciosa|bella|linda|nueva|amplia|moderna|vendo|rento|casa)(?:\s+en\s+venta|\s+sola|\s+nueva|\s+individual|\s+habitacional|\s+residencial|\s+unifamiliar)',
        ...
        'Departamento': [
            r'(?:hermoso|bonito|precioso|bello|lindo|nuevo|amplio|moderno|vendo|rento|departamento)(?:\s+en\s+(?:venta|renta)|\s+nuevo|\s+tipo|\s+estudio)',
        ...
        'Terreno': [
            r'(?:hermoso|bonito|precioso|bello|lindo|nuevo|amplio|vendo|rento|terreno)(?:\s+en\s+(?:venta|renta)|\s+plano|\s+urbano|\s+residencial)',
            r'terreno(?:\s+(?:plano|urbano|residencial))?(?:\s+en\s+(?:venta|renta))?',
            r'lote(?:\s+(?:residencial|urbano|comercial))?',
        ...
        'Local': [
            r'(?:hermoso|bonito|precioso|bello|lindo|nuevo|amplio|moderno|vendo|rento|local)(?:\s+en\s+(?:venta|renta)|\s+comercial|\s+nuevo)',
    ...
    # Si no hay mención explícita, buscar características físicas
    ...
    puntos = {
        'Casa': sum(1 for patron in caracteristicas_vivienda if re.search(patron, texto, re.IGNORECASE)),
        'Departamento': sum(1 for patron in caracteristicas_depto if re.search(patron, texto, re.IGNORECASE)),
        'Terreno': sum(1 for patron in caracteristicas_terreno if re.search(patron, texto, re.IGNORECASE)),
        'Local': sum(1 for patron in caracteristicas_local if re.search(patron, texto, re.IGNORECASE))
    }
...
def extraer_tipo_operacion(texto, precio=None):
    ...
    # Primero verificar el precio si está disponible
    valor_precio = None
    if precio:
        try:
            ...
            if isinstance(precio, dict):
                if "valor" in precio:
                    valor_precio = float(precio["valor"])
                elif isinstance(precio.get("texto"), str):
                    valor_precio = procesar_numero_mexicano(precio["texto"])
            else:
                valor_precio = procesar_numero_mexicano(str(precio))
                
            # Si el precio es mayor a $500k, es venta
            if valor_precio and valor_precio >= 500_000:
                return "venta"
                
            # Si el precio es menor a $30k, es renta
            if valor_precio and valor_precio < 30_000:
                return "renta"
                
        except (ValueError, TypeError):
            pass
    
    # Detectar menciones explícitas de venta
    patrones_venta = [
        r'\b(?:venta|vendo|vendemos|se\s+vende)\b',
        r'\ben\s+venta\b',
        r'\bprecio\s+de\s+venta\b',
        r'\bpropiedad\s+(?:en|de)\s+venta\b',
        r'\bcasa\s+(?:en|de)\s+venta\b',
        r'\bdepartamento\s+(?:en|de)\s+venta\b',
        r'\bterreno\s+(?:en|de)\s+venta\b',
        r'\blocal\s+(?:en|de)\s+venta\b',
        r'\bescrituras?\b',
        r'\bcreditos?\s+(?:bancarios?|hipotecarios?|infonavit|fovissste)\b',
        r'\benganche\b',
        r'\bpie\s+de\s+casa\b',
        r'\bprecio\s+[\d,.]+\s*(?:millones?|mdp|m\.n\.)\b'
    ]
    ...
    # Detectar menciones explícitas de renta
    patrones_renta = [
        r'\b(?:renta|rento|rentamos|se\s+renta)\b',
        r'\ben\s+renta\b',
        r'\bprecio\s+de\s+renta\b',
        r'\bpropiedad\s+(?:en|de)\s+renta\b',
        r'\bcasa\s+(?:en|de)\s+renta\b',
        r'\bdepartamento\s+(?:en|de)\s+renta\b',
        r'\bterreno\s+(?:en|de)\s+renta\b',
        r'\blocal\s+(?:en|de)\s+renta\b',
        r'\barrendamiento\b',
        r'\balquiler\b',
        r'\bmensualidad\b',
    ...
    for patron in patrones_renta:
        if re.search(patron, texto):
            # Verificar que no sea una mención de "renta" en otro contexto
            contexto_no_renta = [
                r'sin\s+renta',
                r'renta\s+de\s+mantenimiento',
                r'cuota\s+de\s+renta',
                r'renta\s+incluida',
                r'incluye\s+renta',
                r'mantenimiento\s+y\s+renta'
            ]
            if not any(re.search(patron_ctx, texto) for patron_ctx in contexto_no_renta):
                return "renta"
    
    # Si no hay mención explícita, usar el precio como criterio adicional
    if valor_precio:
        if valor_precio >= 300_000:
            return "venta"
        elif valor_precio <= 30_000:
            return "renta"
    
    # Si no podemos determinar con certeza, asumir venta para propiedades caras
    if valor_precio and valor_precio >= 500_000:
        return "venta"
    ...
    try:
        ...
        for id_propiedad, datos in datos_crudos.items():
            try:
                ...
                # Si no hay tipo de operación o necesitamos validarlo, usar nuestra función mejorada
                texto_completo = (titulo + " " + descripcion)
                tipo_op = extraer_tipo_operacion(texto_completo, precio_info)
